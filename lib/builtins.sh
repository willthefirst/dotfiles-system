...
source "${BASH_SOURCE%/*}/../../helpers/safe-write.sh"
...
builtin_merge_symlink() {
    ...
    if [[ -d "$last_layer" ]]; then
        ...
        if ! safe_remove_rf "$TARGET"; then
            log_error "Backup failed, aborting symlink: $TARGET"
            return 1
        fi
...
builtin_merge_concat() {
    ...
    if ! safe_remove "$TARGET"; then
        log_error "Backup failed, aborting concat: $TARGET"
        return 1
    fi
    touch "$TARGET"
...
builtin_merge_json() {
    ...
    # Write merged JSON (with backup)
    if ! safe_write_file "$TARGET" "$(echo "$merged" | jq '.')"; then
        return 1
    fi
...
builtin_merge_source() {
    ...
    # Start the target file (with backup)
    if ! safe_write_heredoc "$TARGET" << 'HEADER'
